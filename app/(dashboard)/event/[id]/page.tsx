        ...
        <main className='mx-4 md:w-5/6 md:mx-auto'>
            {!loading && (
                <>
                        ...
                        <div className='grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-x-8'>
                            {event.event_images.slice(0, 4).map((img, index) => (
                                <div key={index} className='aspect-square bg-blue-500 rounded-md overflow-hidden relative'>
                                    <Image
                                        className={`h-full object-cover ${index === event.event_images.slice(0, 4).length - 1 ? 'filter brightness-75' : ''}`}
                    ...
                    <ImageGallery isOpen={isGalleryOpen} onClose={() => setIsGalleryOpen(false)} images={event.event_images} />
