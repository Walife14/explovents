...
function CountryCityDates({ handleSubmit, btnText, includeEventTypes }: Props) {
    ...
    const options = [
        { value: 'boatParty', eventChecked: boatPartyEventChecked, toggleChecked: () => setBoatPartyEventChecked(!boatPartyEventChecked), text: 'Boat Parties' },
        { value: 'poolParty', eventChecked: poolPartyEventChecked, toggleChecked: () => setPoolPartyEventChecked(!poolPartyEventChecked), text: 'Pool Parties' },
        { value: 'beachParty', eventChecked: beachPartyEventChecked, toggleChecked: () => setBeachPartyEventChecked(!beachPartyEventChecked), text: 'Beach Parties' },
        { value: 'otherParty', eventChecked: otherPartyEventChecked, toggleChecked: () => setOtherPartyEventChecked(!otherPartyEventChecked), text: 'Other Parties' },
    ]
    ...
    return (
        <form onSubmit={(e) => handleSubmit(e, country, city, selectedDate, boatPartyEventChecked, poolPartyEventChecked, beachPartyEventChecked, otherPartyEventChecked)}>
            <div className="flex flex-col md:flex-row gap-x-1 gap-y-1 p-1 bg-triary text-2xl rounded-md">
                <div className="md:flex-1 flex">
                    ...
                    <div className="flex items-center justify-center p-2 rounded-l-md bg-white">
                        <svg className="w-8 h-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
            ...
            <div className={`w-72 ${includeEventTypes ? '' : 'w-full text-center mt-4 md:mt-10'}`}>
                <Button text={btnText ? btnText : 'Search'} type="submit" largeText={true} nonFullWidth={true} />
            </div>
        </form>
    )
}
